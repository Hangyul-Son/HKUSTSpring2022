/*
    ...
    Your name:
    Your ITSC email: hson@connect.ust.hk

    Declaration: I declare that I am not involved in plagiarism. I understand that both parties will recieve 0 marks.
...
// Constant and data structure for Gantt chart display
...
void init_cfs_process() {
    ...
    for(int i=0; i<num_process; i++){
        sum_weight += nice_to_weight(nice_value[i]);
    }
    for(int i=0; i<num_process; i++){
        int weight = nice_to_weight(nice_value[i]);
        int time_slice = calculate_per_process_time_slice(weight, sched_latency, sum_weight);
        if(time_slice<min_granularity){
            time_slice=min_granularity;
        }
        process[i].weight = weight;
        process[i].vruntime = 0.0;
        process[i].time_slice = time_slice;
...
void run_cfs_scheduling() {
    ...
    while(finish_process_count<num_process){
        ...
        for(int i=0; i<num_process; i++){
            ...
            if(process[i].vruntime < temp) {
                temp = process[i].vruntime;
                pid = i;
            }
        }
        int duration = process[pid].time_slice;
        process[pid].remain_time -= process[pid].time_slice;
        if(process[pid].remain_time <= 0){
            duration += process[pid].remain_time;
            process[pid].remain_time = 0;
            finish_process_count ++;
        }
        process[pid].vruntime = calculate_new_vruntime(process[pid].vruntime, duration, process[pid].weight);
        gantt_chart_append_item(pid, duration);
    }
    printf(template_step_i, step);
    print_cfs_process();
}
