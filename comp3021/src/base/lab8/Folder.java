...
public class Folder implements Comparable<Folder>, Serializable {
    ...
    public boolean removeNotes(String title) { return notes.removeIf(note -> note.getTitle().equals(title)); };
        ...
        for(Note note: notes){
                ...
                if (word.equals("OR") || word.equals("or")) {
                    if (note instanceof TextNote) {
                        if (note.getTitle().toLowerCase().contains(keywords[i - 1].toLowerCase()) ||
                                ...
                                ((TextNote) note).content.toLowerCase().contains(keywords[i - 1].toLowerCase())) {
                            index[i + 1] = true; index[i - 1] = true; index[i] = true;
                        }
                        else {
                            isSelected = false;
                            continue;
            ...
            for(int i=0; i<keywords.length; i++){
                if(index[i].equals(false)) {
                    if (note instanceof TextNote) {
                        if (note.getTitle().toLowerCase().contains(keywords[i].toLowerCase()) ||
                                ((TextNote) note).content.toLowerCase().contains(keywords[i].toLowerCase())) {
                            index[i] = true;
                        } else {
                            isSelected = false;
                        }
